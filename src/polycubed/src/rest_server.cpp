...
void RestServer::create_metrics() {
  ...
  try {
    ...
    std::vector<std::string> name_services =
        core.get_servicectrls_names_vector();
    ...
    for (size_t i = 0; i < name_services.size(); i++) {
      ...
      auto metrics_service =
          core.get_service_controller(name_services[i]).get_infoMetrics();

      for (size_t j = 0; j < metrics_service.size(); j++) {
        if (metrics_service[j].typeMetric == "C") {
          ...
          map_metrics[name_services[i]].map_counters_.emplace(metrics_service[j].nameMetric,map_metrics[name_services[i]].counters_family_.size()-1);

          // I can access directly to a counter of a family without add a
          // counter counters_family_[i].get().Add({}).Increment();

          // I add a counter to te family
          // In this way I can have a family but many counter with different
          // label=value
         ...
         // map_metrics[name_services[i]].counters_.push_back(  map_metrics[name_services[i]].counters_family_.back().get().Add({}));
          ...
          map_metrics[name_services[i]].map_gauges_.emplace(metrics_service[j].nameMetric,map_metrics[name_services[i]].gauges_family_.size()-1);
...
void RestServer::get_metrics(const Pistache::Rest::Request &request,
  ...
  try {
    ...
    for(auto serviceName: servicesctrls_names)  {
        ...
        //every service has a map with the name of the metric and the InfoMetric struct (we need the pathmetric, typemetric and a value)
